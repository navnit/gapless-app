...
void main() {
  test('launch check returns UpdateAvailable and records timestamp', () async {
    ...
    final result = await _coordinator(
      checker: FakeChecker(_release('0.2.0')),
      prefs: prefs,
    ).checkOnLaunch();
  ...
  test('launch check short-circuits when auto-check disabled', () async {
    final prefs = MemoryPrefs(
      const UpdatePreferencesData(autoCheckEnabled: false),
    );
    final checker = FakeChecker(_release('0.2.0'));
    expect(
      await _coordinator(checker: checker, prefs: prefs).checkOnLaunch(),
      isNull,
    );
    expect(checker.calls, 0);
  });
  ...
  test(
    'launch check suppresses a skipped version but manual still reports it',
    () async {
      final prefs = MemoryPrefs(
        const UpdatePreferencesData(skippedVersion: '0.2.0'),
      );
      expect(
        await _coordinator(
          checker: FakeChecker(_release('0.2.0')),
  ...
  test('up-to-date when latest is not newer', () async {
    expect(
      await _coordinator(
        checker: FakeChecker(_release('0.1.1')),
      ).checkManually(),
      isA<UpToDate>(),
    );
  });
  ...
  test('launch failure is swallowed, manual failure surfaces reason', () async {
    final launch = await _coordinator(
      checker: FakeChecker(
        const UpdateCheckException(CheckFailureReason.rateLimited),
      ),
    ).checkOnLaunch();
